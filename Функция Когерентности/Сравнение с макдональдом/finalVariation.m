...
while (SyPar<=SyReal)
    ...
    disp([' Y = ' num2str(BY), ' SyPar= ', num2str(SyPar), ' SyReal ' num2str(SyReal)] )
    BY = BY+10;
end
BY = BY-1;
...
while (SxPar<=SxReal)
    ...
    SxPar = trapz(x,CoherenceXPar(:,end) );
    SyPar = trapz(y, CoherenceYPar(:,end));
    disp([' X = ' num2str(AX), ' S_par = = ', num2str(SxPar), ' S_real = ' num2str(SxReal), ' Sy_par == ', num2str(SyPar)] )
    AX = AX+10;
end
AX = AX-10;
...
disp([' X = ' num2str(AX), ' S_par = ', num2str(SxPar), ' S_real ' num2str(SxReal)] )
