...
BY = 177;
%CZ = 76;
ax = Inf;%BY/cos(phi);%sqrt(AX^2*BY^2/((AX*cos(phi))^2+(BY*sin(phi))^2));
by = BY;
le = BY/sin(phi);%sqrt(AX^2*BY^2/((AX*sin(phi))^2+(BY*cos(phi))^2));
% ���������������� ������� ���������������
x = -1500:100:1500;
y =  -1000:100:1000;
%x = [-5000:100:5000];
...
FTopFunSAx=ones(size(PsiTopSAx));%1./sqrt(PsiTopSAx);
...
GBotFunSAx = zeros(1,1,length(kd),length(zbot));
GBotFunSAy = ones(1,1,length(kd),length(zbot));

for i =1:length(kd)
    if kd(i)~=0
      %GBotFunSAx(1,1,i,:) = zeros(1, length(zbot));%1i*(ks.^2-1/4*kd(i).^2)*ax^2*dPsiBotSAx(i,:)./(2*kd(i)*PsiBotSAx(i,:));
      GBotFunSAy(1,1,i,:) = 1i*(ks.^2-1/4*kd(i).^2)*by^2*dPsiBotSAy(i,:)./(2*kd(i)*PsiBotSAy(i,:));
    else
      %GBotFunSAx(1,1,i,:) = zeros(1, length(zbot));%sigma2*kplm^4*le*(lt+zbot-zbot.^3/(3*lb^2))./(4*(ks.^2-1/4*kd(i).^2));
      GBotFunSAy(1,1,i,:) = sigma2*kplm^4*le*(lt+zbot-zbot.^3/(3*lb^2))./(4*(ks.^2-1/4*kd(i).^2));
    end
end


FBotFunSAx = ones(length(kd),length(zbot));%1./sqrt(PsiBotSAx);
...
GUnderFunSAx = zeros(1,1,length(kd),length(zunder));
...
for ix = 1:length(x)
    for iy = 1:length(y)
        GammaTop(ix,iy,:,:) = FTopFunSA(1,1,:,:).*exp(-y(iy)^2/by^2*GTopFunSAy(1,1,:,:)).*EXPtopSA(1,1,:,:);
        GammaBot(ix,iy,:,:) = FBotFunSA(1,1,:,:).*exp(-y(iy)^2/by^2*GBotFunSAy(1,1,:,:)).*EXPbotSA(1,1,:,:);
        GammaUnder(ix,iy,:,:) = FUnderFunSA(1,1,:,:).*exp(-y(iy)^2/by^2*GUnderFunSAy(1,1,:,:)).*EXPUnderSA(1,1,:,:);
    end
end
Coherency_Function = cat(4,GammaTop,GammaBot,GammaUnder);

name = ['2D_McDnld_phi' num2str(round(phi/pi*180)) '_AX' num2str(AX) '_BY' num2str(BY) '_sigma' num2str(sigma2) '_fs' num2str(fs/1000000000) 'HHz.mat'];
save(name)